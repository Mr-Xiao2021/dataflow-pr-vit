...
def generate_ds(data_root: str,
    ...
    # Function for configuring dataset for performance
    def configure_for_performance(ds, shuffle_size: int, shuffle: bool = False, cache: bool = False):
        if cache:
            ds = ds.cache()  # Caching improves performance by storing data in memory
        if shuffle:
            ds = ds.shuffle(buffer_size=shuffle_size)  # Shuffle before distributing
        ds = ds.batch(batch_size)
        ds = ds.prefetch(buffer_size=AUTOTUNE)  # Prefetch data to avoid bottlenecks
        return ds

    # Shuffle and batch the training and validation datasets
    ...
    # Distribute datasets if strategy is provided (multi-worker, multi-GPU setup)
